...
from common.utils.manopth.manopth.tensutils import (th_posemap_axisang, th_with_zeros, th_pack, subtract_flat_id, make_list)
...
class ManoLayer(Module):
    ...
    def __init__(self,
        ...
        self.register_buffer('th_betas', torch.Tensor(smpl_data['betas'].r.copy()).unsqueeze(0))
        self.register_buffer('th_shapedirs', torch.Tensor(smpl_data['shapedirs'].r.copy()))
        self.register_buffer('th_posedirs', torch.Tensor(smpl_data['posedirs'].r.copy()))
        self.register_buffer('th_v_template', torch.Tensor(smpl_data['v_template'].r.copy()).unsqueeze(0))
        self.register_buffer('th_J_regressor', torch.Tensor(np.array(smpl_data['J_regressor'].toarray()).copy()))
        self.register_buffer('th_weights', torch.Tensor(smpl_data['weights'].r.copy()))
        self.register_buffer('th_faces', torch.Tensor(smpl_data['f'].copy().astype(np.int32)).long())
    ...
    def forward(
        ...
        if self.use_pca or self.joint_rot_mode == 'axisang':
            ...
            th_full_pose = torch.cat([th_pose_coeffs[:, :self.rot], self.th_hands_mean.to(th_pose_coeffs.device) + th_full_hand_pose], 1)
            ...
            assert th_pose_coeffs.dim() == 4, ('When not self.use_pca, '
                                               'th_pose_coeffs should have 4 dims, got {}'.format(th_pose_coeffs.dim()))
            assert th_pose_coeffs.shape[2:4] == (3, 3), ('When not self.use_pca, th_pose_coeffs have 3x3 matrix for two'
        ...
        if th_betas is None or th_betas.numel() == 1:
            ...
            th_v_shaped = torch.matmul(self.th_shapedirs.to(th_betas.device), th_betas.transpose(1, 0)).permute(2, 0, 1) + self.th_v_template.to(th_betas.device)
            th_j = torch.matmul(self.th_J_regressor.to(th_betas.device), th_v_shaped)
            # th_pose_map should have shape 20x135

        th_v_posed = th_v_shaped + torch.matmul(self.th_posedirs.to(th_pose_map.device), th_pose_map.transpose(0, 1)).permute(2, 0, 1)
        ...
        th_T = torch.matmul(th_results2, self.th_weights.transpose(0, 1).to(th_results2.device))
